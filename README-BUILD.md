...
plain HTML/CSS/JS, no build step needed to serve them: open `site/index.html` in
a browser or drop `site/` on any static host.

## Layout

```
project/        the original Claude Design handoff (unchanged, read-only reference)
build/          the tooling that produced site/
...
site/           the built site — this is the deliverable
```

Rebuild after editing anything in `build/`:

```
python3 build/convert.py && cp build/site.css build/site.js site/
...
Editing pages directly in `site/` is fine too — nothing regenerates unless you
run the converter, and if you do it will overwrite `site/*.html`.
...
they were extracted to `site/assets/blog-*.webp`. Only the images the pages
